...
app.post('/api/saveCustomers', (req, res) => {
  const {
    Name, Address, Orders, Total_Items
  } = req.body;

  // Define the SQL query for inserting into the customers_reservation table
  const query = 'INSERT INTO  `customer` (`Name`, `Address`, `Orders`, `Total_Items`) VALUES (?, ?, ?, ?)';

  // Specify the values to be inserted
  const values = [Name, Address, Orders, Total_Items];
