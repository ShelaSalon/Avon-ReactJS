...
const ReservationForm = () => {
  // State for form fields
  const [name, setName] = useState('');
  const [Caddress, setContactNo] = useState('');
  ...
  // Function to handle menu item selection
  const handleMenuChange = (product, productPrice, quantity) => {
    const existingItemIndex = selectedMenu.findIndex(item => item.product === product);
    ...
    if (existingItemIndex !== -1) {
      ...
      // Add new item
      setSelectedMenu(prevMenu => [...prevMenu, { product, productPrice, quantity }]);
    }
  
    // Calculate total price after updating the menu
    const menuTotal = selectedMenu.reduce((acc, item) => acc + item.productPrice * item.quantity, 0);
    setTotalPrice(menuTotal);
  };

  // Function to handle form submission
  const handleSubmit = () => {
    // Prepare data for the POST request
    const selectedOrder = selectedMenu.map(item => `${item.product} (Quantity: ${item.quantity})`);
   
    const reservationData = {
      Name: name,
      Address: Caddress,
      Orders: selectedOrder.join(', '),
      Total_Items: totalPrice,
    };

    // Send reservation data to the server
    axios.post('http://localhost:8081/api/saveCustomers', reservationData)
      .then(response => {
        console.log('Reservation saved successfully:', response.data);
        // Reset form fields or add logic for success
      })
      .catch(error => {
        console.error('Error saving reservation:', error);
  ...
  return (
    <div>
      ...
      <label>Address:</label>
      <input type="text" value={Caddress} onChange={(e) => setContactNo(e.target.value)} />
          ...
          {menuItems.map((item) => (
            <tr key={item.Product_Name}>
              ...
              <td>
                <input
                  type="checkbox"
                  onChange={(e) => handleMenuChange(item.Product_Name, item.Product_Price, e.target.checked ? 1 : 0)}
                />
                <input
                  type="number"
                  min="0"
                  value={selectedMenu.find((selectedItem) => selectedItem.product === item.Product_Name)?.quantity || 0}
                  onChange={(e) => handleMenuChange(item.Product_Name, item.Product_Price, parseInt(e.target.value, 10))}
                />
              </td>
              <td>
                {selectedMenu.find((selectedItem) => selectedItem.product === item.Product_Name)?.productPrice *
                  selectedMenu.find((selectedItem) => selectedItem.product === item.Product_Name)?.quantity || 0}
              </td>
            </tr>
          ))}
        </tbody>
...
<input
  type="text"
  value={selectedMenu.reduce((acc, item) => acc + item.productPrice * item.quantity, 0)}
  readOnly
/>
      {/* Submit Button */}
      <div className={styles.btnContainer}>
        <div className={styles.buttonSubmit}>
          <button onClick={handleSubmit}>PLACE ORDERS</button>
        </div>
      </div>
    </div>
  );
